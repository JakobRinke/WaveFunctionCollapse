...
function get_current_lowest_realm_cords() {
...
function collapse_neighbour(x, y, j, type) {
    var new_list = [];
...
for (var _ = 0; _<(gamesize-2)*(gamesize-2); _++) {
    var [cur_x, cur_y] =  get_current_lowest_realm_cords();
    if (cur_x == 0 && cur_y == 0) {
        console.log(_);
    ...
    realm[cur_x][cur_y] = [];
    collapse_neighbour(cur_x, cur_y-1, combs[cur_x][cur_y], 0);
    collapse_neighbour(cur_x, cur_y+1, combs[cur_x][cur_y], 1);
    collapse_neighbour(cur_x-1, cur_y, combs[cur_x][cur_y], 2);
    collapse_neighbour(cur_x+1, cur_y, combs[cur_x][cur_y], 3);
}
